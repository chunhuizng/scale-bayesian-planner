...
if __name__ == "__main__":
    ...
    parser.add_argument("--data_path", type=str, default='/data/01/dataset')
    parser.add_argument("--output_file", type=str, default='/data/01/dataset/post_train_data.json')
    args = parser.parse_args()

    vqa_dataset = []
    data_path = args.data_path
    # episodes = list(set(range(1200)) - set(TEST_INDICES)) # if also remove 136, 168, then 
    episodes = list(set(range(1200)) - set(TEST_INDICES) - {136, 168, 222, 255}) # episodes are mainly map to the task's ID
    ...
    for episode in episodes:
        ...
        print(f'Processing task {episode}')
        try:
            task = load_pickles(task_path, folder=True)
        except Exception as e:
            print(f'Error loading task from {task_path}: {e}')
            continue
        num_time = sum(1 for file_name in os.listdir(f'{data_path}/task_{episode}') if file_name.startswith('graph_') and file_name.endswith('.pik')) # in each task_{episode}, we
        # number of time steps, it counts the number of graphs in the task, and it means 
        print(f'Number of time steps: {num_time}')
        ...
        try:
            ...
            print(f"Missing belief {episode}.pik")
            continue
        ...
        try:
            for question_time in range(num_time):
                ...
                # store the action reference and input prompt for each time step in each eposide
                vqa_dataset.append(vqa) # vqa includes {input and ref} multiple num_time; 
                # we also collect the video each question time
                video = task[f'video_{question_time}.pik']
        except Exception as e:
            print(f"Missing graph {episode}.pik")
